...
function getM1(params, I_buf, s_buf, A_buf, B_buf, K_buf) {
  ...
  var hashg = crypto.createHash(params.hash).update(params.g.toBuffer()).digest();
  var hashI = crypto.createHash(params.hash).update(I_buf).digest();
  var hashK = crypto.createHash(params.hash).update(K_buf).digest();

  var XOR_buf = bignum.fromBuffer(hashN)
    .xor(bignum.fromBuffer(hashg)).toBuffer();

  return crypto.createHash(params.hash)
    .update(XOR_buf).update(hashI).update(s_buf)
    .update(A_buf).update(B_buf).update(K_buf).digest();
}
...
function Client(params, salt_buf, identity_buf, password_buf, secret1_buf) {
  ...
  this._private.I_buf = identity_buf;
  this._private.s_buf = salt_buf;
...
Client.prototype = {
  ...
  setB: function setB(B_buf) {
    ...
    p.M1_buf = getM1(p.params, p.I_buf, p.s_buf, p.A_buf, B_buf, p.K_buf);
...
function Server(params, salt_buf, identity_buf, verifier_buf, secret2_buf) {
  if (!(this instanceof Server))  {
    return new Server(params, salt_buf, identity_buf, verifier_buf, secret2_buf);
  }
  assertIsBuffer(salt_buf, "salt (salt)");
  assertIsBuffer(identity_buf, "identity (I)");
...
Server.prototype = {
  ...
  setA: function setA(A_buf) {
    ...
    p.K_buf = getK(p.params, S_buf);
    p.M1_buf = getM1(p.params, p.I_buf, p.s_buf, A_buf, p.B_buf, p.K_buf);
